...
@Component({
  ...
  templateUrl: './company-update-exercise.component.html',
  styleUrls: ['./company-update-exercise.component.css']
})

export class CompanyUpdateExerciseComponent implements OnInit {
  ...
  updateExercise() {
    ...
    if ((document.getElementById('software') as HTMLInputElement).checked === true) {
      environment.push('Software testting');
    }
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          response.json().then(errorJson => {
            document.getElementById('output').innerText = 'Internship is the same as before or fields haven\'t been properly filled in!';
